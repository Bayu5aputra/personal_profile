...
const ReviewSection = ({ productId, onReviewAdded }) => {
  ...
  const loadReviews = async () => {
    ...
    try {
      ...
      const dist = await getRatingDistribution(productId);
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate key data
    const keyValidation = await validateKey(formData.keyData);
    ...
    const alreadyReviewed = await hasUserReviewed(productId, formData.name);
    if (alreadyReviewed) {
    ...
    try {
      // Mark key as used
      const keyUsed = await useKey(formData.keyData, formData.name, productId);
      if (!keyUsed) {
